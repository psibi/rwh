...
-- usage: stack nested_command_option.hs -- -help
...
main = do
  ...
  where
    ...
    programOptions =
      ...
      (hsubparser ((createCommand <> deleteCommand)) <|> pure Delete)
    createCommand :: Mod CommandFields Command
    createCommand = command "create" (info copt (progDesc "Create a thing"))
    deleteCommand :: Mod CommandFields Command
    deleteCommand =
      command "delete" (info (pure Delete) (progDesc "Delete the thing"))
    createOptions :: Parser CreateOpts
    createOptions =
      CreateOpts <$> switch (long "some-flag" <> help "Set the flag") <*>
      switch (long "ome-flag" <> help "Set the flag") <*>
